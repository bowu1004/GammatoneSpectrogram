...
if __name__ == '__main__':
    ...
    # --- TBD ---#
    folder_names = [join(main_dir, x) for x in listdir(main_dir) if isdir(x) and not x.startswith('.') and not x.startswith('_')]
    folder_names = natsort.natsorted(folder_names, reverse=False)
    if len(folder_names) == 0:
        print('Warning: no folders. Please add folders.')
    try:
        folder_name_Record = []
        file_count_Record = []
        for folder_index in range(len(folder_names)):  # 5
            # --- In each categorical folder ---#
            print('\nProcessing... {:.0f}%, \n   {}'.format(100 * (folder_index + 1) / len(folder_names),
                                                            folder_names[folder_index]))
            folder_name_Record.append(folder_names[folder_index])
            audio_filenames = [join(folder_names[folder_index], x) for x in listdir(folder_names[folder_index]) if
                               is_audio_file(x)]
            ...
            if len(audio_filenames) == 0:
                print('      Warning: no audio files in folder: {}'.format(folder_names[folder_index]))
            for audio_index in range(len(audio_filenames)):
                ...
                one_sec_segment = seg_aud_1s_no_padding(sig, start=0, end=-1, sr=sample_rate_2D, window=1,
                ...
                for k in range(one_sec_segment.__len__()):
                    ...
                    render_GFCC_from_sig(sig=one_sec_segment[k], sample_rate_2D=sample_rate_2D, duration=False,
                                         function=gammatone.gtgram.gtgram, pngFile_name=pngFile_name)

    except:
        #--- error and continue ---#
        print('\n !!!! Error occurred!')
        print(folder_names, folder_index, audio_filenames, audio_index)
        pass
